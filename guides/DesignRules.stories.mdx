...
- Each component receives a `size` prop whose value can be one of these: `xs`, `sm`, `md`, `lg`, `xl`, `2xl`, `3xl`, `4xl`, `5xl`, `6xl`, `7xl`, `8xl`, & `9xl`. You just have to pass `size` prop with one of these value and you're done.
- Another example could be `color` prop. You can pass one of the colors defined in `brandColor` of theme to any component.
- If it's a layout based component like `Box`, `Stack`, `Inline` or our best friend `Card` then it receives a `space` prop. This also takes one of the values between `xs` and `9xl` ( with an addition of `space="none"`) and provide appropriate spacing between components.
...
- **textColor:** "default", "heading", "gray", "subtle", "disabled", and "white"
...
- The available sizes you can use is `xs`, `sm`, `md`, `lg`, `xl`, `2xl`, `3xl`, `4xl`, `5xl`, `6xl`, `7xl`, `8xl`, & `9xl`. The default is `"md"` and it will be applied if are not passing anything.
- Font size is also similar and it ranges from "xs" to "9xl".

## Space

Space also shares similar API as size but with one addition. You can also pass `"none"` with the range of `"xs"` to `"9xl"`. Space is a dedicated prop for layout components like `Stack`, `Inline`, `Box` and `Card`.
